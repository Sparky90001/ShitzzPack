...
''')
print(f'''{Style.BRIGHT}{Fore.CYAN}                            Token Stock: {len(open('input/3m_tokens.txt', 'r').read().splitlines())} Boost Stock: {len(open('input/3m_tokens.txt', 'r').read().splitlines()) * 2}''' + Fore.RESET)	

def menu():
    invite = input(f"{Style.BRIGHT}{Fore.GREEN}[+]{Fore.BLUE} Invite: ")
    if ".gg/" in invite:
    ...
    try:
        months = int(input(f"{Style.BRIGHT}{Fore.BLUE}[+]{Fore.BLUE} Months: "))
    except:
    ...
    try:
        amount = (input(f"{Style.BRIGHT}{Fore.BLUE}[+] {Fore.BLUE}Amount: "))
    except:
    ...
    if amount % 2 != 0:
        sprint("Amount must be Even", False)
        return
    nick = input(f"{Style.BRIGHT}{Fore.MAGENTA}[+] {Fore.BLUE}Nickname: ")
    go = time.time()
