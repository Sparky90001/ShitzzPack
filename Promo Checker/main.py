...
class colors:
  ...
  def ask(qus):
    print(
      f"{Fore.LIGHTMAGENTA_EX}[?]{Fore.RESET}{Style.BRIGHT} {qus}{Fore.RESET}{Style.NORMAL}"
    )

  def what(txt):
    print(
      f"{Fore.LIGHTBLUE_EX}[?]{Fore.RESET}{Style.BRIGHT} {txt}{Fore.RESET}{Style.NORMAL}"
    )

  def banner(txt):
    print(
      f"{Fore.LIGHTMAGENTA_EX}{Style.BRIGHT}{txt}{Fore.RESET}{Style.NORMAL}")

  def error(txt):
    print(
      f"{Fore.RED}[{random.choice(['-', '!'])}]{Fore.RESET}{Style.DIM} {txt}{Fore.RESET}{Style.NORMAL}"
    )

  def sucess(txt):
    print(
      f"{Fore.GREEN}[+]{Fore.RESET}{Style.BRIGHT} {txt}{Fore.RESET}{Style.NORMAL}"
    )

  def warning(txt):
    print(
      f"{Fore.LIGHTYELLOW_EX}[!]{Fore.RESET}{Style.DIM} {txt}{Fore.RESET}{Style.NORMAL}"
    )

  def log(txt):
    print(
      f"{Fore.LIGHTMAGENTA_EX}[!]{Fore.RESET}{Style.BRIGHT} {txt}{Fore.RESET}{Style.NORMAL}"
    )
  ...
  def ask3(qus):
    print(
      f"{Fore.LIGHTBlUE_EX}[+]{Fore.RESET}{Style.BRIGHT} {qus}{Fore.RESET}{Style.NORMAL}"
    )


clear()
title("Promo Checker - addi")
clear()
bnr = pyfiglet.figlet_format("addi")
colors.banner(bnr + "\n")
colors.ask("Delay")
delay = int(input(">>> "))
colors.ask("Chill\n")
    ...
    async with cs.get(
      ...
      if rs.status in [200, 204, 201]:
        ...
        if data["uses"] == data["max_uses"]:
          ...
          colors.sucess(
            f"Valid -> {promocode} | Days Left: {days} | Expires At: {exp_at} | Title: {title}"
          )
          save("valid.txt",
               f"https://discord.com/billing/promotions/{promocode}")
      elif rs.status == 429:
